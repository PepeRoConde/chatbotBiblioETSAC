...
\lstdefinestyle{terminal}{
    ...
    basicstyle=\ttfamily\small\color{rosa},
    frame=single,
    breaklines=true,
...
pero fortemente interconectadas. Por unha banda, o sistema de \emph{crawling} \ref{fig:drawio},
...
\emph{non relevante}) a partir de \textit{keywords}\footnote{Pódese vela presenza destas keywords, e por consiguiente o efecto de poda do sistema sobre a árbore da web no Apéndice \ref{app:relDoc}}. A pesar da súa simplicidade,
...
Atopáronse 10.164.265 palabras cun vocabulario de 73.640 termos, das coles 15.889 aparecen só unha vez (\textit{hapax legomena}), un 20\% do vocabulario. A maioría non son erros: só 230 son faltas ortográficas e 30 xeradas polo OCR. Este fenómeno de vocabulario de uso desigual segue a lei de Zipf, onde poucas palabras son moi frecuentes e moitas son raras. Para unha análise máis detallada das distribucións de frecuencias e lonxitude de documentos, véxase o Apéndice~\ref{app:zipf_pareto}.
...
densa (mediante embeddings semánticos e similitude euclidiana en FAISS) e busca
léxica (usando BM25, unha evolución do TF-IDF). As puntuacións de ambos métodos
fusiónanse con pesos determinados empiricamente, seleccionando os $k$ fragmentos
de texto máis relevantes da base de documentos da universidade.
...
entre axentes e o seguimento de instrucións complexas \cite{claude-agents-2024}.
O modelo xera así unha resposta precisa fundamentada na documentación oficial.

O sistema enriquece cada documento recuperado coa procedencia da información, 
procedencias (hipervínculos), permitindo a verificación das fontes utilizadas en cada resposta.
...
(.txt) polo \textit{parse} do \textit{crawler}. Cada documento é fragmentado en varios
\textit{chunks} de tamaño configurable, establecéndose tamén un \textit{overlap}
seleccionable que especifica cantos \textit{tokens} comparten os fragmentos consecutivos. A
...
nese caso o LLM podería non comprender o contexto completo.

Antes da fragmentación, cada documento é enriquecido cun breve texto de contexto
(\textit{pre-chunk}) que se antepón ao contido principal. Este texto inclúe
información básica como o título do documento, o hipervínculo de orixe e a
data da última modificación (nas bandeiras de execución do programa, este modo chámase \textcolor{gray}{\texttt{source}}). O seu obxectivo é proporcionar ao modelo un contexto
mínimo e consistente sobre a procedencia do fragmento, mellorando a interpretación
durante a xeración da resposta. Considerouse a posibilidade de xerar este contexto
de forma máis elaborada mediante un LLM, mais debido ao gran volume de documentos
e ao custo computacional asociado, optouse finalmente por unha solución máis
sinxela e eficiente, aínda que o programa permite o seu uso.

Para o almacenamento e indexación dos \textit{embeddings}, emprégase
FAISS (\textit{Facebook AI Similarity Search}) \cite{johnson2019billion}, unha
biblioteca especializada na busca de similaridade en espazos vectoriais de alta
dimensionalidade. No noso caso, o índice utilizado é de tipo \textit{IndexFlatL2},
o que realiza unha busca exacta (brute-force) dos veciños máis próximos, calculando
as distancias de forma determinista sen aproximacións. Cada \textit{chunk} de texto
convértese nun vector denso (\textit{embedding}) mediante o modelo
\textit{all-MiniLM-L6-v2} da familia Sentence-Transformers \cite{wang-etal-2024-searching}. Este modelo está baseado
na arquitectura MiniLM, unha versión destilada e optimizada de BERT, e foi
entrenado mediante a técnica Sentence-BERT para a xeración de embeddings semánticos.
Estes vectores almacénanse, nunha estrutura de índice
optimizada; e cando se realiza unha consulta, FAISS calcula os vectores máis similares
utilizando medidas de distancia euclidiana~\cite{douze2024faiss}.
Só se xeran novos \textit{embeddings} para aqueles documentos que o \textit{crawler}
identifica como modificados, permitindo un reprocesado incremental eficiente.
O índice FAISS pódese reconstruír totalmente de cada vez, un gasto
computacional asumible que garante a consistencia da base de vectores.

\subsection{O sistema de recuperación}

O sistema polo que o RAG recibe os documentos relevantes para cada consulta
recibe múltiples iteracións e cambios tras varias fases de probas e
...
\cite{systematic_rag_2025}.

Primeiro de todo, explorouse unha aproximación baseada en métodos clásicos de
recuperación léxica, en particular TF-IDF \cite{sparck_jones1972}. Este enfoque
permitía identificar termos discriminativos dentro da colección e ofrecía unha
interpretación directa das puntuacións obtidas. Porén, durante a experimentación
observouse que TF-IDF presentaba puntuacións significativamente máis baixas en
comparación coa recuperación densa baseada en FAISS, o que complicaba a súa combinación
directa. Ademais, a súa sensibilidade a termos frecuentes obrigaba a introducir
estratexias de normalización ou ponderación para poder fusionar puntuacións con
FAISS.

Estas limitacións motivaron a adopción de BM25, un modelo que mellora o TF-IDF
para recuperación léxica, aplicando normalización por lonxitude de documento
e saturación da frecuencia de termos, obtendo resultados máis robustos e compatibles
coa recuperación densa \cite{robertson1995okapi}.

Como experimento adicional, implementouse un \textit{reranker} básico baseado
na identificación das 25 palabras máis frecuentes da colección, tratándoas como
\textit{stopwords} dinámicas para penalizar documentos excesivamente xerais. Aínda
que este enfoque mostrou resultados prometedores, non chegou a ser explotado
en profundidade debido ao enfoque posto na evolución cara a \textit{TILDEv2} e
ás limitacións temporais da práctica (véxase a Sección~\ref{sec:extensiones}
para unha discusión sobre estratexias avanzadas de reranking).

Inicialmente, o sistema empregou un método de recuperación baseado exclusivamente
en vectores FAISS mediante busca por similitude euclidiana \cite{langchainfaiss}. Porén, durante as primeiras probas
comprobouse que esta aproximación podía non ofrecer os resultados esperados,
xa que daba a mesma importancia a todas as palabras da consulta, sen ter en conta
cal podería ser a palabra clave máis relevante. A maioría das consultas, especialmente
aquelas de carácter burocrático, conteñen moitas palabras comúns e repetidas ao
longo da colección de documentos, o que podía facer que o sistema recuperase documentos
pouco relevantes.

Como primeira mellora, experimentouse con MMR (Maximal Marginal Relevance), un
algoritmo que busca documentos relevantes evitando redundancia entre eles. A idea
subxacente era que, deste xeito, non se recuperarían documentos redundantes entre si,
xa que se existisen varios documentos moi similares, a súa información adicional
para a consulta sería limitada. Non obstante, durante a experimentación observouse
que esta suposición non se cumpría no noso dominio. É moi común atopar diferentes
documentos oficiais que repiten a mesma información, ou varias versións e revisións
dun proceso en distintos documentos. Neses casos, o sistema tendía a devolver só
un deles, perdéndose información relevante ou, incluso, indicando un proceso xa
obsoleto, ao priorizar a diversidade fronte á exhaustividade \cite{carbonell1998mmr}.
Esta problemática, relacionada coa vixencia temporal dos documentos, constitúe un
dos maiores retos deste proxecto (e de calquera sistema RAG que manipule información
cambiante ao longo do tempo \cite{grofsky2025freshness}) e será tratada con maior
detalle máis adiante.

Descartado MMR, como solución óptima (aínda que o programa permite o seu uso), a solución final adoptada consistiu en manter similitude euclidiana sobre vectores densos e incorporar BM25, co que xa se experimentara e investigara inicialmente, como
compoñente léxico adicional á recuperación de FAISS, creando así un sistema
híbrido que combina ambas estratexias mediante pesos configurables. Posteriormente axustáronse empiricamente xunto co mellor tamaño de
\textit{chunk} mediante un \textit{pipeline} de validación (véxase a subsección sobre o uso do mecanismo de validación como avaliador para máis detalles sobre este proceso de optimización).

Ademais, o fluxo de recuperación intégrase dentro dunha canle máis ampla de
procesamento da consulta. Antes de realizar a búsqueda, lévase a cabo unha
\textit{clasificación da consulta} (mediante un LLM lixeiro, como Claude Haiku)
para determinar se é necesario recuperar documentos. En caso afirmativo, aplícase
un \textit{reescrito da consulta} (\textit{query rewriting}), co obxectivo de optimizar
a súa formulación para a busca tanto no índice denso (FAISS) como no léxico (BM25).
Os documentos recuperados por este sistema híbrido, xunto coa consulta orixinal,
pásanlle finalmente a un modelo de linguaxe máis grande (como Claude Sonnet) para
xerar a resposta final.
...
precisión, tempo de resposta e custo. \ref{fig:flujo}
...
\subsection{Xestión de vixencia dos documentos}
Como xa foi mencionado anteriormente, o problema de tratar cun \textit{corpus}
destas caracterísitcas é averiguar se un documento dispoñible foi derrogado ou
non, é moi común que as páxinas que os manteñen non sexan borradas e sigan
mostrándoo como se seguise tendo relevancia. Tal problemática non é única a esta
aplicación en concreto, pois supón un dos grandes campos de investigación dos
\textit{Ragsystems}. 
...
accesible (o server da páxina non o mostra), a responsabilidade de 
xestionar a vixencia dos documentos recae no LLM, isto conseguese con enxeñaría de \texttt{prompts}, engadindo a
obriga de intentar sempre dar unha data a partir do 
...
documentos que non dispoñen de \textit{last\_modified} teñen algunha data neles
...
É dicir, licencias de \emph{open-source} pouco restrictivas \ . Adicionalmente a nosa implementación do BM25 baseouse nun Gist de GitHub  \cite{koreyou2019bm25}. Como os Gist son GPL, polo menos o noso Sistema de procesamento de vectores tamén debe ser GPL.
...
O sistema foi desenvolvido e probado en Python nas versións 3.12 a 3.14, garantindo
compatibilidade con estas versións do intérprete. Para instalar e executar o chatbot,
...
python -m src.crawl # Lanza o crawler e xera o corpus de documentos
python -m src.main # Lanza o RAGSystem e construe a base de vectores
\end{lstlisting}

\textbf{Nota importante:} A primeira execución do sistema tardará un tempo considerable,
xa que é necesario completar dous procesos intensivos: primeiro, o \textit{crawler}
debe percorrer e descargar todos os documentos dos portais web da universidade; segundo,
o sistema debe xerar e almacenar os \textit{embeddings} de todos os fragmentos de texto
na base de vectores FAISS. Execucións posteriores serán significativamente máis rápidas,
xa que o \textit{crawler} só actualizará documentos modificados e os \textit{embeddings}
xa estarán dispoñibles na caché. Unha vez finalizado este proceso inicial, o chatbot
estará listo para responder consultas de forma interactiva.
...
\subsection{Uso do mecanismo de validación como avaliador}
...
\textit{crawler} especializado para a recolección automática de documentación
...
mediante métricas clásicas de RI e avaliación da xeración con \textit{LLM-as-judge},
...
\begin{figure}[h]
  \centering
    \includegraphics[width=0.9\textwidth]{imaxes/tabla-ocr.drawio.png}
    \caption{Funcionamento do OCR das Táboas. Para que sexa lexible para un LLM.}
    \label{fig:ocrTable}
\end{figure}
...
fila 1: [Tipos de usuarios]-GRUPO 1 [Tipos de usuarios]-GRUPO 1 [de documentos en préstamo]-GRUPO 1 [Dias de]-GRUPO 1 [Renovaciones]-GRUPO 1 [Reservas]-GRUPO 1
fila 2: [Tipos de usuarios]-Estudiantado de Graco de centros adscritos [Tipos de usuarios]-Estudiantado de Graco de centros adscritos [de documentos en préstamo]-10 [Dias de]-10 dias [Renovaciones]-Indefinidas [Reservas]-Limite 10 does,
fila 3: [Tipos de usuarios]-Estudiantado de programa de movilidad [Tipos de usuarios]-(Erasmus, Sicue-Séneca) [de documentos en préstamo]-10 [Dias de]-10 dias [Renovaciones]-Indefinidas [Reservas]-Limite 10 does,
fila 4: [Tipos de usuarios]-Estudiantada de doble Grado, de simultaneidad [Tipos de usuarios]-Estudiantada de doble Grado, de simultaneidad [de documentos en préstamo]-10 [Dias de]-10 dias [Renovaciones]-Indefinidas [Reservas]-Limite 10 does,
fila 5: [Tipos de usuarios]-Estudiantado de grados interuniversitarios [Tipos de usuarios]-Estudiantado de grados interuniversitarios [de documentos en préstamo]-10 [Dias de]-10 dias [Renovaciones]-Indefinidas [Reservas]-Limite 10 does,
fila 6: [Tipos de usuarios]-Estudiantada da Universidad Sénior [Tipos de usuarios]-Estudiantada da Universidad Sénior [de documentos en préstamo]-10 [Dias de]-10 dias [Renovaciones]-Indefinidas [Reservas]-Limite 10 does,
fila 7: [Tipos de usuarios]-Estudiantado de MASTERES posgrados propios, los dela Universidade da Corufia [Tipos de usuarios]-Estudiantado de MASTERES posgrados propios, los dela Universidade da Corufia [de documentos en préstamo]-15 [Dias de]-21 dias [Renovaciones]-Indefinidas [Reservas]-Limite 15 does,
fila 8: [Tipos de usuarios]-Estudiantada de Trabajos de fin de grado [Tipos de usuarios]-Estudiantada de Trabajos de fin de grado [de documentos en préstamo]-15 [Dias de]-21 dias [Renovaciones]-Indefinidas [Reservas]-Limite 15 does,
fila 9: [Tipos de usuarios]-Personal de administracion en servicio (PTGAS) [Tipos de usuarios]-Personal de administracion en servicio (PTGAS) [de documentos en préstamo]-15 [Dias de]-21 dias [Renovaciones]-Indefinidas [Reservas]-Limite 15 does,
fila 10: [Tipos de usuarios]-Estudiantada de [Tipos de usuarios]-Estudiantada de [de documentos en préstamo]-35 [Dias de]-30 dias [Renovaciones]-Indefinidas [Reservas]-Limite 85 does,
fila 11: [Tipos de usuarios]-Personal investigador visitante: visitante visitante postdactoral [Tipos de usuarios]-Personal investigador visitante: visitante visitante postdactoral [de documentos en préstamo]-35 [Dias de]-30 dias [Renovaciones]-Indefinidas [Reservas]-Limite 85 does,
fila 12: [Tipos de usuarios]-Becarins/as de investigacion [Tipos de usuarios]-Becarins/as de investigacion [de documentos en préstamo]-35 [Dias de]-Curso académico (cada podré excluir de este tipo de préstamo por razén de uso y disponibilidad) [Renovaciones]-Indefinidas [Reservas]-Limite 85 does,
fila 13: [Tipos de usuarios]-Personal investigador [Tipos de usuarios]-Personal investigador [de documentos en préstamo]-35 [Dias de]-Curso académico (cada podré excluir de este tipo de préstamo por razén de uso y disponibilidad) [Renovaciones]-Indefinidas [Reservas]-Limite 85 does,
fila 14: [Tipos de usuarios]-GRUPOS [Tipos de usuarios]-GRUPOS [de documentos en préstamo]-GRUPOS [Dias de]-GRUPOS [Renovaciones]-GRUPOS [Reservas]-GRUPOS
fila 15: [Tipos de usuarios]-PDI da UDC (incluyendo centros adscritas), de la Fundacion UDC [Tipos de usuarios]-PDI da UDC (incluyendo centros adscritas), de la Fundacion UDC [de documentos en préstamo]-100 [Dias de]-Curso académico (cada podré excluir de este tipo de préstamo por razén de uso y disponibilidad) [Renovaciones]-Indefinidas [Reservas]-Limite 100 does.
fila 16: [Tipos de usuarios]-Profesorado emérito, jubilado incentivado honorario [Tipos de usuarios]-Profesorado emérito, jubilado incentivado honorario [de documentos en préstamo]-100 [Dias de]-Curso académico (cada podré excluir de este tipo de préstamo por razén de uso y disponibilidad) [Renovaciones]-Indefinidas [Reservas]-Limite 100 does.
fila 17: [Tipos de usuarios]-Profesorado visitante [Tipos de usuarios]-Profesorado visitante [de documentos en préstamo]-100 [Dias de]-Curso académico (cada podré excluir de este tipo de préstamo por razén de uso y disponibilidad) [Renovaciones]-Indefinidas [Reservas]-Limite 100 does.
fila 18: [Tipos de usuarios]-Lectores/as [Tipos de usuarios]-Lectores/as [de documentos en préstamo]-100 [Dias de]-Curso académico (cada podré excluir de este tipo de préstamo por razén de uso y disponibilidad) [Renovaciones]-Indefinidas [Reservas]-Limite 100 does.
fila 19: [Tipos de usuarios]-GRUPOG [Tipos de usuarios]-GRUPOG [de documentos en préstamo]-GRUPOG [Dias de]-GRUPOG [Renovaciones]-GRUPOG [Reservas]-GRUPOG
fila 20: [Tipos de usuarios]-Cualquier persona la comunidad universitaria de la UDC. que sea autorizada por la Biblioteca Universitaria [Tipos de usuarios]-Cualquier persona la comunidad universitaria de la UDC. que sea autorizada por la Biblioteca Universitaria [de documentos en préstamo]-None [Dias de]-10 dias [Renovaciones]-Indefinidas [Reservas]-Limite does,
\end{spacing}
\end{tcolorbox}
...
\begin{figure}[H]
  \centering
    \includegraphics[width=\textwidth]{imaxes/crawl_speed.png}
    \caption{Progresión da eficiencia do \textit{crawler} ao longo do tempo}
    \label{fig:crawlerRun}
\end{figure}

\section{Lei de Zipf e Principio de Pareto no corpus}
\label{app:zipf_pareto}
...
\begin{figure}[H]
    ...
    \caption{ ~Distribución de tipos de ficheiros no corpus. Observamos que, malia a web estar constituída por documentos HTML, está tan poboada de documentos de normativa en formato PDF que a razón e casi cinconta cinconta.}
    \label{fig:file_types_distribution}
\end{figure}


\section{Relevancia dos documentos}
\label{app:relDoc}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{imaxes/numero_palabras_clave_documentos.png}
    \caption{ ~Presencia de palabras chave nos documentos \emph{crawleados}. Pódese ver que o mecanismo implementado de descartar documentos sen palabras chave realmente supón unha gran poda na árbore da web da UDC.}
    \label{fig:file_types_distribution}
\end{figure}
...
\chapter{Diagramas acerca da estrutura do RAGsystem}

\section{Diagrama de fluxo do RAG}
\begin{figure}[h]
  \centering
    \includegraphics[width=0.7\textwidth]{imaxes/crawler-processing.png}
    \caption{Diagrama de fluxo do RAG UDC}
    \label{fig:drawio}
\end{figure}

\section{Diagram de comunucación de axentes do \textit{RAG}}
\begin{figure}[h]
  \centering
    \includegraphics[width=0.7\textwidth]{imaxes/flujo.png}
    \caption{Comunicación entre axentes do RAG UDC}
    \label{fig:flujo}
\end{figure}
...
\begin{figure}[h]
  \centering
    ...
    \caption{Distribución de datas presentes no contido dos documentos sen \texttt{last\_modified}}
    \label{fig:tempohist}
\end{figure}
